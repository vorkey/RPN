...
function consonantCounterRecursive(sentences) {
    let arrFilter = ["a", "i", "u", "e", "o", " "];
    let res = 0;

    if (sentences == "") return res;
    
    if (!arrFilter.includes(sentences[0].toLowerCase()) && !Number.parseInt(sentences[0])) {
        res += 1;
    }
    return res + consonantCounterRecursive(sentences.slice(1));
}
