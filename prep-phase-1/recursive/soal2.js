...
function changeXRecursive(data, jenis) {
    const regexGenap = /[24680]/;
    const regexGanjil = /[13579]/;
    let tempStr = data;
    const regex = jenis == "ganjil" ? regexGenap : regexGanjil;

    let r = data.search(regex);
    if (r == -1) {
        return data;
    } else {
        return changeXRecursive(tempStr.replace(regex, "x"), jenis);
    }
}
