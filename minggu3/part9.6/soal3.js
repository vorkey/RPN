...
dimana array tersebut berisi objek-objek barang dari toko X tersebut yang berisikan 
info nama barang, siapa saja yang membeli, sisa stock barang dan total pemasukan untuk barang tersebut
*/
...
function countProfit(shoppers) {
  ...
  for (let i = 0; i < shoppers.length; i++) {
    for (let j = 0; j < resArray.length; j++) {
      if (shoppers[i].product == resArray[j].product && shoppers[i].amount <= resArray[j].leftOver) {  
        resArray[j].shoppers.push(shoppers[i].name);
        resArray[j].leftOver -= shoppers[i].amount;
        resArray[j].totalProfit += listBarang[j][1] * shoppers[i].amount;
      } else {
        continue;
      }
    }
